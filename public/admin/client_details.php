...
<?php
  if(empty($_GET['id'])) {
    $session->message("Aucun ID client n'a été fournit.");
    redirect_to('index.php');
  }
  
  $client = Client::find_by_id($_GET['id']);
  if(!$client) {
    $session->message("Le client n'a pas été retrouvé.");
    redirect_to('index.php');
  }

  $notPaidFacutres = Facture::find_not_paid_by_id($client->id);
  $dette = 0;
  foreach($notPaidFacutres as $facture): 
    $dette += $facture->dette;
  endforeach;
  $paidFacutres    = Facture::find_paid_by_id($client->id);
    ...
    <tbody>
      <tr>
        ...
        <td>
          ...
          <a href="delete_client.php?id=<?php echo $client->id; ?>"><input type="button" value="Supprimer" class="btn-danger"></a>
        </td> 
      </tr>
    </tbody>
</table>
